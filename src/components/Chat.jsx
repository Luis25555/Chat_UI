...
export default function Chat({ selectedUserId,onProfilePicClick, onProfileClick, isMobile}) {
...
//rendering starts here
  return (
  <div className={`bg-white shadow-md rounded-md p-4 flex flex-col max-h-[93%] h-[93%] ${isMobile ? 'w-[35%] ml-0' : ''}`}>
    {selectedUser && (
      <div className="flex justify-between items-center mb-4 bg-purple-100  ">
        ...
        <div className="flex items-center gap-4">
          <img
            ...
            className="w-12 h-12 rounded-full object-cover cursor-pointer ml-2"
          />
          <div>
            <h2 className="text-xs font-bold text-black md:text-lg">{selectedUser.username}</h2>
            <p className="!text-xs text-gray-500 md:text-sm">{selectedUser.position}</p>
          </div>
        </div>
    ...
    {/* Prompts if no user is selected */}
    {!selectedUser && selectedUserId && (
      <h2 className="text-xl text-black font-bold mb-4">Chat with User {selectedUserId}</h2>
    )}
    {!selectedUserId && (
            ...
            <div className="max-w-[70%]">
              ...
              {!isMe && (
                <div className="flex items-center gap-2 text-xs mb-1">
                  ...
                  />
                  <div>
                    <span className="font-semibold">{selectedUser?.username || "Unknown"}</span>
                ...
                <div className="break-words whitespace-pre-wrap ">
                  ...
                  {msg.image && (
                    <img
                      src={msg.image}
                      alt="attachment"
                      className="mt-2 max-w-[150] md:max-w-[300px] md:align-middle rounded shadow"
                    />
                  )}
                </div>
              </div>
